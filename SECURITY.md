...
- By email: **ishe@vambo.ai**, subject line starting `lensa security`.

Please include the lensa version (`lensa --version`), the OS, the Chromium and
ffmpeg that `lensa doctor` reports, the smallest op script or command line that
...
Supported versions: the latest release only. lensa is pre-1.0 and fixes land
on the tip rather than being backported.

## The trust boundary, stated honestly

lensa is a tool for driving a browser with an automation protocol and turning
...
### `lensa serve --port` is a remote control socket

This is the one that matters most, so it comes first.

`lensa serve --port N` listens on `127.0.0.1:N` and executes the op protocol
...
- **Binding to loopback is not a boundary against a web browser.** Any web
  page the user visits while lensa is serving can issue a cross-origin
...
Therefore: prefer `lensa serve` on stdin, which is the default and has no
...
- **The op script.** A `.jsonl` script is a program. It can navigate to
  ...
  same way. Do not feed lensa a script from a pull request, an issue body, or
  any other place a stranger can write.
- **The environment.** `LENSA_CHROMIUM`, `LENSA_CHROMIUM_ARGS`, `LENSA_FFMPEG`,
  `LENSA_SPOOL_DIR`, `TMPDIR` and `PATH` all steer which binaries lensa
  executes and where it writes. An attacker who can set your environment or
  prepend to your `PATH` has already won; lensa does not try to defend against
  that.
- **The browser profile.** lensa launches Chromium with a fresh throwaway
...
- **The page being recorded.** lensa points a browser at a URL. That page runs
  its own JavaScript in a normal Chromium sandbox. lensa adds no isolation
...
### What lensa does try to get right
...
- Argument injection into the processes lensa spawns. Output and input paths
...
- Anything that causes lensa to execute a binary other than the one the user
...
- **The telemetry sidecar contains every URL the take visited.** It is off
  unless `--keep-temp` or `LENSA_TELEMETRY` asks for it, and it then records
...
- Use `lensa serve` on stdin. If `--port` is unavoidable, bind it on a host
